...
def compute_indicators(raw_data, mcap_data, screen_tickers):
    ...
    # ADV in INR Cr (1 Cr = 1e7)
    adv = ((volume * close) / 1e7).rolling(ADV_PERIOD, min_periods=ADV_PERIOD).mean()
    print('✓')
...
def find_screen_date(ind):
    # Anchor on Nifty index + highly-liquid mega-caps that are virtually
    # guaranteed to have data for any completed trading day (avoids picking
    # a stale date just because some smaller/illiquid tickers lag in
    # yfinance's batch response).
    anchors = [t for t in ('^NSEI', 'RELIANCE.NS', 'TCS.NS') if t in ind['close'].columns]
    if anchors:
...
def run_screen(ind):
    ...
    result = pd.DataFrame({
        ...
        'adv_m'         : adv_row[pt].values,   # ₹ Cr (field name kept for schema parity)
        'mcap_m'        : mcap_row[pt].values,  # ₹ Cr
